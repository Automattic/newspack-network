...
class TestMembershipEventsNullPlan extends WP_UnitTestCase {
	...
	/**
	 * Test that membership_transferred returns null (no fatal) when plan is missing.
	 */
	public function test_membership_transferred_returns_null_when_plan_missing() {
		$new_owner      = $this->factory->user->create_and_get( [ 'user_email' => 'new@example.com' ] );
		$previous_owner = $this->factory->user->create_and_get( [ 'user_email' => 'prev@example.com' ] );

		$membership = new Mock_User_Membership( $new_owner, null, 44 );

		$result = Memberships_Events::membership_transferred( $membership, $new_owner, $previous_owner );

		$this->assertNull( $result, 'membership_transferred should return null when plan is missing, not fatal.' );
	}
}
